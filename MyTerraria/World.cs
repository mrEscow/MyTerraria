...
//отвечает за логику и прорисовку мира

namespace MyTerraria
{
    class World : Transformable, Drawable
    {
        //количество чангов по ширине и высоте
        ...
        public void SetTile(TileType type, int i, int j)
        {
            ...
            chunk.SetTite(type, tilePos.X, tilePos.Y, upTile, downTile, leftTile, rightTile);
        ...
        public Tile GetTile( int x, int y)
        {
            ...
            if (chunk == null) //если чанг не определен
                return null;   //то возвращаем null

            // Получаем позицию плитки в массиве чанка
            var tilePos = GetTilePosFromChunk(x, y);

            // Возвращаем плитку даже если она равна null
            return chunk.GetTite( tilePos.X, tilePos.Y);
        ...
        public void Draw(RenderTarget target, RenderStates states)
        {
            ...
            for (int x = 0; x < WORLD_SIZE; x++)
            {
                ...
                {
                    //если чанг не определен то пропускаем ее прорисовку
